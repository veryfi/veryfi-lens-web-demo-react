...
export class WasmWrapper {
    ...
    setDocumentCallback(callback) {
        ...
        this.documentDetectorLoaded = this.wasmModule.ccall('setDocumentDetectorCallback', 'boolean', ['number'], [documentCallback]);
    }
    ...
    getStitchedImage() {
        ...
        let { data, blurLevel , outputHeight, outputWidth } = this.getResultFromBuffer(outputBuffer);
    ...
    getResultFromBuffer(outputBuffer) {
        let struct = Array.from({length: 3}, (_, i) => new DataView(new Uint8Array(this.wasmModule.HEAPU8.subarray(outputBuffer, outputBuffer + 12)).buffer).getInt32(i * 4, true));
        let [matAddress, outputHeight, outputWidth] = struct;
        let blurLevel = this.wasmModule.HEAPF32[(outputBuffer + 12) / Float32Array.BYTES_PER_ELEMENT];
        const bufferData = this.wasmModule.HEAPU8.subarray(matAddress, matAddress + outputHeight * outputWidth * 4);
